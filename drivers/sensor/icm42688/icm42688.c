...
static const uint16_t icm42688_gyro_sensitivity_x10[] = {
	1310, 655, 328, 164
};

/* see "Accelerometer Measurements" section from register map description */
static void icm42688_convert_accel(struct sensor_value *val,
				   int16_t raw_val,
...
static void icm42688_convert_gyro(struct sensor_value *val,
				  int16_t raw_val,
...
static inline void icm42688_convert_temp(struct sensor_value *val,
					 int16_t raw_val)
{
...
static int icm42688_channel_get(const struct device *dev,
				enum sensor_channel chan,
				struct sensor_value *val)
{
	const struct icm42688_data *drv_data = dev->data;

	switch (chan) {
	case SENSOR_CHAN_ACCEL_XYZ:
		icm42688_convert_accel(val, drv_data->accel_x,
				       drv_data->accel_sensitivity_shift);
		icm42688_convert_accel(val + 1, drv_data->accel_y,
				       drv_data->accel_sensitivity_shift);
		icm42688_convert_accel(val + 2, drv_data->accel_z,
				       drv_data->accel_sensitivity_shift);
		break;
	...
	case SENSOR_CHAN_ACCEL_Y:
		icm42688_convert_accel(val, drv_data->accel_y,
				       drv_data->accel_sensitivity_shift);
		break;
	case SENSOR_CHAN_ACCEL_Z:
		icm42688_convert_accel(val, drv_data->accel_z,
				       drv_data->accel_sensitivity_shift);
		break;
	case SENSOR_CHAN_GYRO_XYZ:
		icm42688_convert_gyro(val, drv_data->gyro_x,
				      drv_data->gyro_sensitivity_x10);
		icm42688_convert_gyro(val + 1, drv_data->gyro_y,
				      drv_data->gyro_sensitivity_x10);
		icm42688_convert_gyro(val + 2, drv_data->gyro_z,
				      drv_data->gyro_sensitivity_x10);
		break;
	...
	case SENSOR_CHAN_GYRO_Y:
		icm42688_convert_gyro(val, drv_data->gyro_y,
				      drv_data->gyro_sensitivity_x10);
		break;
	case SENSOR_CHAN_GYRO_Z:
		icm42688_convert_gyro(val, drv_data->gyro_z,
				      drv_data->gyro_sensitivity_x10);
		break;
	case SENSOR_CHAN_DIE_TEMP:
		icm42688_convert_temp(val, drv_data->temp);
		break;
	default:
		return -ENOTSUP;
	}
...
int icm42688_tap_fetch(const struct device *dev)
{
	int result = 0;
...
static int icm42688_sample_fetch(const struct device *dev,
				 enum sensor_channel chan)
{
	int result = 0;
...
static int icm42688_attr_set(const struct device *dev,
			     enum sensor_channel chan,
...
static int icm42688_attr_get(const struct device *dev,
			     enum sensor_channel chan,
			     enum sensor_attribute attr,
			     struct sensor_value *val)
{
...
static int icm42688_data_init(struct icm42688_data *data,
			      const struct icm42688_config *cfg)
{
	data->accel_x = 0;
	data->accel_y = 0;
	data->accel_z = 0;
...
static int icm42688_init(const struct device *dev)
{
	...
	icm42688_spi_init(drv_data->spi, &drv_data->spi_cfg);
	icm42688_data_init(drv_data, cfg);
	icm42688_sensor_init(dev);

	drv_data->accel_sensitivity_shift = 14 - 3;
	drv_data->gyro_sensitivity_x10 = icm42688_gyro_sensitivity_x10[3];

	if (icm42688_init_interrupt(dev) < 0) {
...
#define ICM42688_DEFINE_CONFIG(index)					\
	static const struct icm42688_config icm42688_cfg_##index = {	\
...
#define ICM42688_INIT(index)						\
	ICM42688_DEFINE_CONFIG(index);					\
	static struct icm42688_data icm42688_driver_##index;		\
	DEVICE_DT_INST_DEFINE(index, icm42688_init,			\
			    NULL,					\
			    &icm42688_driver_##index,			\
			    &icm42688_cfg_##index, POST_KERNEL,		\
