...
int icm42688_set_fs(const struct device *dev, uint16_t a_sf, uint16_t g_sf)
{
	uint8_t databuf;
	int result;
...
int icm42688_set_odr(const struct device *dev, uint16_t a_rate, uint16_t g_rate)
{
	uint8_t databuf;
	int result;
...
int icm42688_turn_on_sensor(const struct device *dev)
{
	...
	icm42688_set_fs(dev, drv_data->accel_sf, drv_data->gyro_sf);

	icm42688_set_odr(dev, drv_data->accel_hz, drv_data->gyro_hz);
